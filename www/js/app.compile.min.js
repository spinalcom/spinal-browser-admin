...
compareTo.$inject=[];function compareTo(){return{require:"ngModel",scope:{compareTolValue:"=compareTo"},link:function(a,b,c,d){d.$validators.compareTo=function(b){return b==a.compareTolValue},a.$watch("compareTolValue",function(){d.$validate()})}}}angular.module("app.directives").directive("navbar",[function(){return{restrict:"E",templateUrl:"app/templates/navbar.html",controller:"navbarCtrl"}}]).directive("menuGlayout",["goldenLayoutService","$timeout",function(a,b){return{restrict:"E",scope:{layoutInfo:"=info"},replace:!0,template:"<li ng-repeat=\"layout in layoutInfo\"  id=\"{{layout.id}}\"><a >{{layout.name}}</a></li>",link:(c)=>{a.wait_ready().then(()=>{let d=(a,b)=>()=>{a.createChild(b.cfg)};a.watch_pannel((e)=>{console.log("in watch_pannel :"),console.log(e),b(()=>{for(var b=0;b<e.length;b++){let c=e[b];c.shown||(c.shown=!0,a.createDragSource($("#"+c.id)[0],c.cfg),$("#"+c.id).click(d(a,c)))}},200),c.layoutInfo=e})})}}}]).directive("ngRightClick",["$parse",function(a){return function(b,c,d){var e=a(d.ngRightClick);c.bind("contextmenu",function(a){b.$apply(function(){a.preventDefault(),e(b,{$event:a})})})}}]).directive("compareTo",compareTo).provider("$copyToClipboard",[function(){this.$get=["$q","$window",function(a,b){var c=angular.element(b.document.body),d=angular.element("<textarea/>");return d.css({position:"fixed",opacity:"0"}),{copy:function(e){var f=a.defer();f.notify("copying the text to clipboard"),d.val(e),c.append(d),d[0].select();try{var g=b.document.execCommand("copy");if(!g)throw g;f.resolve(g)}catch(a){f.reject(a)}finally{d.remove()}return f.promise}}}]}]);
