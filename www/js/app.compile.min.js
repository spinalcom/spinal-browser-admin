...
angular.module('app.spinalcom').factory('spinalModelDictionary',['$q','ngSpinalCore','config','authService',function(a,b,c,d){function e(a){for(var b=0;b<j.length;b++)j[b].reject(a)}function f(a){for(var b=0;b<j.length;b++)j[b].resolve(a)}let g={model:0,users:0},h=!1,j=[];return g.init=()=>d.wait_connect().then(()=>{var c=a.defer();if(0!=g.users)c.resolve(g.model);else if(j.push(c),1===j.length){d.get_user();b.load_root().then((a)=>(g.model=a,b.load('/etc/users')),()=>{let a='not able to load : /';console.error(a),e(a)}).then((a)=>{g.users=a,h=!1,f(g.model)},()=>{let a='not able to load : /etc/users';console.error(a),e(a)})}return c.promise},()=>{let a='not able to load : /';console.error(a),e(a)}),g}]);
